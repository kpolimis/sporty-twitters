...
class api(object):
	...
	def classifyUser(self, users_dir, uids,
			...
			while not uids.empty():
				...
				if l1 != l2:  # sporty tweets have been removed
					if not sporty:  # user is not supposed to be exercising
						logging.debug("no_sport user %s is exercising" % uid)
				...
				if not filtered_utweets:
					logging.debug("no tweets for %s" % uid)
					continue
				# filter out non-english users
				user = filtered_utweets[0]['user']
				if user['lang'] != 'en':
					logging.debug("user %s lang is not en" % uid)
					continue
		...
		for i in range(proc_count):
			processes[i] = Process(target=classify_one_user,args=(sport_hash, auto_hash, requested, classifiers, users_dir, uids_q, label_names, scores, probability))
		for p in processes:
			p.start()
		for p in processes:
			p.join()
