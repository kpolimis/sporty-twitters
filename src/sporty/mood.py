...
class api(object):
	...
	def classifyUser(self, users_dir, uids,
		...
		def filter_on_poms(tw):
			...
			if requested.intersection(words):
				return True
			return False

		def classify_one_user(sport_hash, auto_hash, requested, classifiers,
				      users_dir, uids, label_names,
				      probability, i, stdout_lock):
			...
			while True:
				...
				if len(utweets.tolist()) != len(no_sport_utweets):  # sporty tweets have been removed
				...
				score_denom = len(filtered_utweets)

				if requested:
					poms_tweets = filter(filter_on_poms, filtered_utweets)
					filtered_utweets = poms_tweets
		...
		for uid in uids:
			uids_q.put(uid)
		# Add empty task to the task queue so that the workers exit
		for i in range(proc_count):
			uids_q.put(None)
		for i in range(proc_count):
			p = Process(target=classify_one_user,
				    args=(sport_hash, auto_hash, requested,
					  classifiers, users_dir, uids_q,
					  label_names, probability, i, stdout_lock))
			processes.append(p)
			p.start()
		for p in processes:
			p.join()
